...
*/
    ...
    pub fn from_pciconf(slot: &PciSlot) -> Option<PciDevice> {
        let out = cmd!(pciconf ("-l") (format!("pci0:{}:{}:{}", slot.bus, slot.slot, slot.func))).output().unwrap();
    ...
    pub fn from_pciconf_l() -> Vec<PciDevice> {
        let output = std::process::Command::new("pciconf").arg("-l").output().unwrap();
    ...
    pub fn from_pciconf_l_line(line: &str) -> PciDevice
    {
        ...
        let domain = num[0];
        let slot = PciSlot { bus: num[1], slot: num[2], func: num[3] };
